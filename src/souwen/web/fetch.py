...
函数/类清单：
    ...
    _fetch_with_provider(provider, urls, timeout) -> FetchResponse
        ...
        - 关键变量：分支 jina_reader / arxiv_fulltext / tavily / firecrawl / exa / metaso /
...
def _filter_fetch_handler_kwargs(
    ...
    """Keep provider-private kwargs compatible with older fetch handlers."""
    if not kwargs:
        return kwargs
    try:
    ...
    accepted.discard("urls")
    accepted.discard("timeout")
...
def _get_provider_global_timeout(provider: str, url_count: int, timeout: float) -> float:
    ...
    ``metaso`` 逐条调用 Reader API，也按 URL 数量放大总预算。
    ``xcrawl`` 使用并发度 3 的 semaphore 逐条抓取，按 ceil(url_count/3) 轮放大。
    """
    if provider in {"arxiv_fulltext", "metaso"}:
...
def ssrf_blocked_fetch_result(
    ...
    *,
    ...
    """Return a blocked FetchResult when a provider-level URL safety check fails."""
...
def raise_if_fetch_url_blocked(url: str) -> None:
    """Raise a clear error when a direct URL API cannot represent blocked results."""
    ok, reason = validate_fetch_url(url)
    if not ok:
...
def split_fetch_urls_by_ssrf(
    ...
    *,
    ...
    """Split provider batch URLs into safe URLs and blocked result placeholders."""
    safe_urls: list[str] = []
...
async def _handle_metaso(urls: list[str], timeout: float, **_kwargs: Any) -> FetchResponse:
    from souwen.web.metaso import MetasoClient

    async with MetasoClient() as client:
        results: list[FetchResult] = []
        for url in urls:
            response = await client.reader(url, timeout=timeout)
...
register_fetch_handler("metaso", _handle_metaso)
...
def _fetch_provider_adapters_by_name():
...
async def fetch_content(
    ...
    Args:
        urls: 目标 URL 或 URL 列表；字符串会归一化为单元素列表
        providers: 提供者或提供者列表；字符串会归一化为单元素列表，默认 ["builtin"]
    ...
    if not skip_ssrf_check:
        ...
        valid_entries = list(enumerate(normalized_urls))
    ...
    if strategy == "fanout":
        ...
        provider_results: list[FetchResult] = []
        for provider, response in zip(selected, responses, strict=False):
            provider_results.extend(
                _align_results_to_urls(
                    provider=provider,
                    urls=valid_urls,
    ...
    all_results = [
        final_by_index[index] for index in range(len(normalized_urls)) if index in final_by_index
    ]
    ok_count = sum(1 for result in all_results if result.error is None)
