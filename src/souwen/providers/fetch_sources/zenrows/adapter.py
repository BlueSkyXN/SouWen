"""Canonical ZenRows Fetch bridge over the SSRF-safe existing client."""
...
from .spec import ZENROWS_FETCH_PROFILE


class ZenRowsClientProtocol(Protocol):
    async def fetch(self, url: str, timeout: float = 30.0) -> Any: ...
    async def close(self) -> None: ...


class ZenRowsFetchProvider(ClientFetchProvider):
    capability = "fetch"

    def __init__(self, client: ZenRowsClientProtocol, *, enabled: bool = True) -> None:
...
def _project(receipt: Any, request: FetchTargetRequest, context: RequestContext) -> FetchResult:
    del context
    return _result(receipt, request, ZENROWS_FETCH_PROFILE.provider_id)
...
def _target(request: FetchTargetRequest) -> str:
    ...
    if not validate_fetch_url(target)[0]:
        raise ProviderError(
            ProviderErrorCode.POLICY_BLOCKED, provider_id=ZENROWS_FETCH_PROFILE.provider_id
        )
    return target
...
_FETCH_SPEC = ClientFetchSpec(ZENROWS_FETCH_PROFILE.provider_id, _invoke, _project)

__all__ = ["ZenRowsClientProtocol", "ZenRowsFetchProvider"]
