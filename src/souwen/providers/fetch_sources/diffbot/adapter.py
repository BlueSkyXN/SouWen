...
class DiffbotClientProtocol(Protocol):
...
def _project(receipt: Any, request: FetchTargetRequest, context: RequestContext) -> FetchResult:
    del context
    return _result(receipt, request, DIFFBOT_FETCH_PROFILE.provider_id)
...
def _target(request: FetchTargetRequest) -> str:
    ...
    if not validate_fetch_url(target)[0]:
        raise ProviderError(
            ProviderErrorCode.POLICY_BLOCKED, provider_id=DIFFBOT_FETCH_PROFILE.provider_id
        )
    return target
