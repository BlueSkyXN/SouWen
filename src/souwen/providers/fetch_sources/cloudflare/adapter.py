"""Canonical Cloudflare Browser Rendering Fetch bridge."""
...
from .spec import CLOUDFLARE_FETCH_PROFILE
...
class CloudflareFetchProvider(LegacyFetchProvider):
    capability = "fetch"

    def __init__(self, client: CloudflareClientProtocol, *, enabled: bool = True) -> None:
...
def _project(receipt: Any, request: FetchTargetRequest, context: RequestContext) -> FetchResult:
    del context
    return _result(receipt, request, CLOUDFLARE_FETCH_PROFILE.provider_id)
...
def _target(request: FetchTargetRequest) -> str:
    ...
    if not validate_fetch_url(target)[0]:
        raise ProviderError(
            ProviderErrorCode.POLICY_BLOCKED, provider_id=CLOUDFLARE_FETCH_PROFILE.provider_id
        )
    return target
...
_FETCH_SPEC = LegacyFetchSpec(CLOUDFLARE_FETCH_PROFILE.provider_id, _invoke, _project)

__all__ = ["CloudflareClientProtocol", "CloudflareFetchProvider"]
