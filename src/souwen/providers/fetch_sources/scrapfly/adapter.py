"""Canonical Scrapfly Fetch bridge over the SSRF-safe existing client."""
...
from .spec import SCRAPFLY_FETCH_PROFILE


class ScrapflyClientProtocol(Protocol):
    async def fetch(self, url: str, timeout: float = 30.0) -> Any: ...
    async def close(self) -> None: ...


class ScrapflyFetchProvider(ClientFetchProvider):
    capability = "fetch"

    def __init__(self, client: ScrapflyClientProtocol, *, enabled: bool = True) -> None:
...
def _project(receipt: Any, request: FetchTargetRequest, context: RequestContext) -> FetchResult:
    del context
    return _result(receipt, request, SCRAPFLY_FETCH_PROFILE.provider_id)
...
def _target(request: FetchTargetRequest) -> str:
    ...
    if not validate_fetch_url(target)[0]:
        raise ProviderError(
            ProviderErrorCode.POLICY_BLOCKED, provider_id=SCRAPFLY_FETCH_PROFILE.provider_id
        )
    return target
...
_FETCH_SPEC = ClientFetchSpec(SCRAPFLY_FETCH_PROFILE.provider_id, _invoke, _project)

__all__ = ["ScrapflyClientProtocol", "ScrapflyFetchProvider"]
