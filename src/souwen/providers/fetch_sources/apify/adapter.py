"""Canonical Apify Fetch bridge over the SSRF-safe existing client."""
...
from .spec import APIFY_FETCH_PROFILE
...
class ApifyFetchProvider(ClientFetchProvider):
    capability = "fetch"

    def __init__(self, client: ApifyClientProtocol, *, enabled: bool = True) -> None:
...
def _project(receipt: Any, request: FetchTargetRequest, context: RequestContext) -> FetchResult:
    del context
    return _result(receipt, request, APIFY_FETCH_PROFILE.provider_id)
...
def _target(request: FetchTargetRequest) -> str:
    ...
    if not validate_fetch_url(target)[0]:
        raise ProviderError(
            ProviderErrorCode.POLICY_BLOCKED, provider_id=APIFY_FETCH_PROFILE.provider_id
        )
    return target
...
_FETCH_SPEC = ClientFetchSpec(APIFY_FETCH_PROFILE.provider_id, _invoke, _project)

__all__ = ["ApifyClientProtocol", "ApifyFetchProvider"]
