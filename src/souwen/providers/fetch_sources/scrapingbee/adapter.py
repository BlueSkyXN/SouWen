"""Canonical ScrapingBee Fetch bridge over the SSRF-safe existing client."""
...
from .spec import SCRAPINGBEE_FETCH_PROFILE


class ScrapingBeeClientProtocol(Protocol):
    async def fetch(self, url: str, timeout: float = 30.0) -> Any: ...
    async def close(self) -> None: ...


class ScrapingBeeFetchProvider(ClientFetchProvider):
    capability = "fetch"

    def __init__(self, client: ScrapingBeeClientProtocol, *, enabled: bool = True) -> None:
...
def _project(receipt: Any, request: FetchTargetRequest, context: RequestContext) -> FetchResult:
    del context
    return _result(receipt, request, SCRAPINGBEE_FETCH_PROFILE.provider_id)
...
def _target(request: FetchTargetRequest) -> str:
    ...
    if not validate_fetch_url(target)[0]:
        raise ProviderError(
            ProviderErrorCode.POLICY_BLOCKED, provider_id=SCRAPINGBEE_FETCH_PROFILE.provider_id
        )
    return target
...
_FETCH_SPEC = ClientFetchSpec(SCRAPINGBEE_FETCH_PROFILE.provider_id, _invoke, _project)

__all__ = ["ScrapingBeeClientProtocol", "ScrapingBeeFetchProvider"]
