"""Provider v2 bridge for zhipuai's legacy web Search client."""
...
_PROVIDER_ID = "zhipuai"


class ZhipuAISearchClientProtocol(Protocol):
    async def search(self, query: str, max_results: int = 10) -> Any: ...


class ZhipuAISearchSearchProvider(LegacySearchProvider):
    def __init__(self, client: ZhipuAISearchClientProtocol, *, enabled: bool = True) -> None:
...
_SPEC = LegacySearchSpec(_PROVIDER_ID, "web", _invoke, _project)

__all__ = ["ZhipuAISearchClientProtocol", "ZhipuAISearchSearchProvider"]
