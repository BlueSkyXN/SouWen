"""Provider v2 bridges for firecrawl Search and Fetch."""
...
_PROVIDER_ID = "firecrawl"


class FirecrawlClientProtocol(Protocol):
    async def search(self, query: str, max_results: int = 10) -> Any: ...
    async def scrape(self, url: str, timeout: float = 30.0) -> Any: ...
    async def close(self) -> None: ...


class FirecrawlSearchProvider(LegacySearchProvider):
    def __init__(self, client: FirecrawlClientProtocol, *, enabled: bool = True) -> None:
        super().__init__(client, _SEARCH_SPEC, enabled=enabled)


class FirecrawlFetchProvider(LegacyFetchProvider):
    def __init__(self, client: FirecrawlClientProtocol, *, enabled: bool = True) -> None:
...
async def _fetch(client: Any, request: FetchTargetRequest) -> Any:
    ...
    return await client.scrape(target, timeout=30.0)
