"""Provider v2 bridges for kimi_code Search and Fetch."""
...
_PROVIDER_ID = "kimi_code"


class KimiCodeClientProtocol(Protocol):
    async def search(self, query: str, max_results: int = 10) -> Any: ...
    async def fetch(self, url: str, timeout: float = 30.0) -> Any: ...
    async def close(self) -> None: ...


class KimiCodeSearchProvider(ClientSearchProvider):
    def __init__(self, client: KimiCodeClientProtocol, *, enabled: bool = True) -> None:
        super().__init__(client, _SEARCH_SPEC, enabled=enabled)


class KimiCodeFetchProvider(ClientFetchProvider):
    def __init__(self, client: KimiCodeClientProtocol, *, enabled: bool = True) -> None:
...
async def _fetch(client: Any, request: FetchTargetRequest) -> Any:
    ...
    return await client.fetch(target, timeout=30.0)
