"""Provider v2 bridge for feishu_drive's existing web Search client."""
...
_PROVIDER_ID = "feishu_drive"


class FeishuDriveClientProtocol(Protocol):
    async def search(self, query: str, max_results: int = 10) -> Any: ...


class FeishuDriveSearchProvider(ClientSearchProvider):
    def __init__(self, client: FeishuDriveClientProtocol, *, enabled: bool = True) -> None:
...
_SPEC = ClientSearchSpec(_PROVIDER_ID, "office", _invoke, _project)

__all__ = ["FeishuDriveClientProtocol", "FeishuDriveSearchProvider"]
