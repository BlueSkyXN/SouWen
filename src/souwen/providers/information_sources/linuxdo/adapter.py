"""Provider v2 bridge for linuxdo's existing web Search client."""
...
_PROVIDER_ID = "linuxdo"


class LinuxDoClientProtocol(Protocol):
    async def search(self, query: str, max_results: int = 10) -> Any: ...


class LinuxDoSearchProvider(ClientSearchProvider):
    def __init__(self, client: LinuxDoClientProtocol, *, enabled: bool = True) -> None:
...
_SPEC = ClientSearchSpec(_PROVIDER_ID, "cn_tech", _invoke, _project)

__all__ = ["LinuxDoClientProtocol", "LinuxDoSearchProvider"]
