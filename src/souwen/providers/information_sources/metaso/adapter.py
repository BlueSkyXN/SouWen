"""Provider v2 bridges for metaso Search and Fetch."""
...
class MetasoClientProtocol(Protocol):
    async def search(self, query: str, max_results: int = 10) -> Any: ...
    async def reader(self, url: str, timeout: float = 30.0) -> Any: ...
    async def close(self) -> None: ...


class MetasoSearchProvider(ClientSearchProvider):
    def __init__(self, client: MetasoClientProtocol, *, enabled: bool = True) -> None:
        super().__init__(client, _SEARCH_SPEC, enabled=enabled)


class MetasoFetchProvider(ClientFetchProvider):
    def __init__(self, client: MetasoClientProtocol, *, enabled: bool = True) -> None:
...
async def _fetch(client: Any, request: FetchTargetRequest) -> Any:
    ...
    return await client.reader(target, timeout=30.0)
