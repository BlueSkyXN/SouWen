"""Provider v2 bridges for tavily Search and Fetch."""
...
class TavilyClientProtocol(Protocol):
    ...
    async def extract(self, urls: list[str], timeout: float = 30.0) -> Any: ...
    async def close(self) -> None: ...


class TavilySearchProvider(ClientSearchProvider):
    def __init__(self, client: TavilyClientProtocol, *, enabled: bool = True) -> None:
        super().__init__(client, _SEARCH_SPEC, enabled=enabled)


class TavilyFetchProvider(ClientFetchProvider):
    def __init__(self, client: TavilyClientProtocol, *, enabled: bool = True) -> None:
...
async def _fetch(client: Any, request: FetchTargetRequest) -> Any:
    ...
    return await client.extract([target], timeout=30.0)
