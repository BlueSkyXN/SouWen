"""Provider v2 bridge for serper's legacy web Search client."""
...
class SerperClientProtocol(Protocol):
    async def search(self, query: str, max_results: int = 10) -> Any: ...


class SerperSearchProvider(LegacySearchProvider):
    def __init__(self, client: SerperClientProtocol, *, enabled: bool = True) -> None:
...
__all__ = ["SerperClientProtocol", "SerperSearchProvider"]
