...
logger = logging.getLogger("souwen.providers.runtime_clients.web.youtube")
...
def resolve_api_key(existing_field: str, env_var: str) -> str | None:
    """解析 YouTube API Key

    依次尝试：频道配置（sources.youtube.api_key）→ 旧版扁平字段
    （SouWenConfig.{existing_field}）→ 环境变量。任一命中即返回。

    暴露为模块级函数主要便于测试时通过 ``monkeypatch.setattr(
    "souwen.providers.runtime_clients.web.youtube.resolve_api_key", ...)`` 直接替换。
    """
    cfg = get_config()
    key = cfg.resolve_api_key("youtube", existing_field)
    if key:
        return key
