...
文件用途：
    ...
    本模块使用 SouWen 已有基础设施（httpx / jina_reader）在
...
模块依赖：
    ...
    - souwen.providers.runtime_clients.models: FetchResult, FetchResponse
    - souwen.providers.runtime_clients.web.jina_reader: JinaReaderClient（Markdown 提取）

技术要点：
    - 域安全：只允许 deepwiki.com 域
    - 输入归一化：完整 URL / owner/repo 简写 / 单词关键词 均可使用
    - 库名解析：通过 GitHub Search API 解析单词关键词（对齐 resolveRepoFetch.ts）
    - 直接通过 jina_reader 获取 Markdown
    - 空内容保护：提取失败时返回包含 error 的 FetchResult
"""
...
logger = logging.getLogger("souwen.providers.runtime_clients.web.deepwiki")
...
class DeepWikiClient:
    ...
    使用 SouWen 的 Jina Reader 提取策略替代原 TypeScript 实现。

    Args:
        github_token: GitHub Token（用于关键词解析，可选）
        jina_api_key: Jina Reader API Key（可选）
    """
    ...
    async def fetch(
        ...
        logger.info("DeepWiki 抓取: url=%s", url)

        # Jina Reader（cloud Markdown 提取，可绕过 JS 渲染）
        ...
        return FetchResult(
            ...
            final_url=url,
            source=self.PROVIDER_NAME,
            error="DeepWiki 抓取失败：jina_reader 未返回内容",
