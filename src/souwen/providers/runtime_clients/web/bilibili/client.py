...
模块依赖：
    - urllib.parse：URL 编码
    - souwen.providers.runtime_clients.models：WebSearchResponse / WebSearchResult
    - souwen.common_runtime.provider_support.scraper.base：BaseScraper
    - souwen.config：可选 SESSDATA / bili_jct 读取
    - souwen.providers.runtime_clients.web.bilibili.wbi：WBI 签名器
    - souwen.providers.runtime_clients.web.bilibili.models：B 站详情数据模型
    - souwen.providers.runtime_clients.web.bilibili._errors：错误码 → 异常映射
...
from souwen.providers.runtime_clients.web.bilibili._errors import raise_for_code
...
logger = logging.getLogger("souwen.providers.runtime_clients.web.bilibili")
