...
文件用途：
    ...
    （souwen.providers.runtime_clients.web._html_extract.extract_from_html）转换为 Markdown / Text，
...
模块依赖：
    ...
    - souwen.common_runtime.provider_support.exceptions: ConfigError 异常
    - souwen.common_runtime.provider_support.http_client: SouWenHttpClient HTTP 客户端基类
    - souwen.providers.runtime_clients.models: FetchResponse, FetchResult 数据模型
    - souwen.providers.runtime_clients.web._html_extract: 共享 HTML→Markdown/Text 提取工具
...
from souwen.providers.runtime_clients.models import FetchResponse, FetchResult
from souwen.providers.runtime_clients.web._html_extract import extract_from_html

logger = logging.getLogger("souwen.providers.runtime_clients.web.scrapingbee")
...
class ScrapingBeeClient(SouWenHttpClient):
    ...
    async def fetch(self, url: str, timeout: float = 30.0) -> FetchResult:
        ...
        """
        from souwen.providers.runtime_clients.web.fetch import ssrf_blocked_fetch_result
