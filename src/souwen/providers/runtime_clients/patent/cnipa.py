...
模块依赖：
    - httpx: HTTP 异步客户端
    - souwen.common_runtime.provider_support.http_client: OAuth 连接管理
    - souwen.providers.runtime_clients.models: 统一数据模型（PatentResult, SearchResponse 等）
    - souwen.common_runtime.provider_support.rate_limiter: 限流控制（令牌桶算法）
    - souwen.config: 配置管理（读取 API 凭证）
    - souwen.common_runtime.provider_support.exceptions: 异常类
"""
...
from souwen.common_runtime.provider_support.exceptions import ConfigError, NotFoundError, ParseError
from souwen.common_runtime.provider_support.http_client import OAuthClient
