...
Type: Guardrail card.
This directory contains admin-only config, proxy, WARP and source-management endpoints.
Read `src/souwen/server/auth.py`, the target admin route, `tests/test_server/test_app.py` and relevant config docs before editing.
Read this card for any admin route, state-changing endpoint, secret-bearing response or management behavior.

## Why this is high-risk

- Admin endpoints can mutate runtime config, HTTP backend/proxy settings and WARP behavior.
