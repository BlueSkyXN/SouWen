"""Runtime source provenance helpers.

Release containers and binary bundles expose the immutable Git commit through
``SOUWEN_SOURCE_SHA`` or a ``runtime.source.sha`` file.  Local source runs do not
need provenance and therefore return ``None`` when neither source is present.
"""
...
def _default_source_sha_files() -> Iterator[Path]:
    ...
    candidates.extend(
        (
            module_path.parents[1] / SOURCE_SHA_FILENAME,
            module_path.parents[2] / SOURCE_SHA_FILENAME,
        )
    )
