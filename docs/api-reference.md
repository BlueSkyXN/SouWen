...
`research_output:search`，当前为 `datacite`；返回 `ResearchOutputResult`，保留 source 的
...
`research_output:search` 派生，当前为匿名 DataCite REST API。每条结果保留
...
DataCite 的 `landing_url` 与声明的 `content_urls` 不证明目标可访问、可抓取、可下载或可再分发；
SouWen 不跟随这些链接，不下载内容。`rights_list` 会保留多个上游权利/许可陈述，不能被降格为
单个“开放”布尔值。
