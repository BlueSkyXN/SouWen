...
```python
from souwen import search, search_papers, search_research_outputs, search_patents, web_search
```
...
| `domain` | `str` | `"paper"` | `"paper"` / `"research_output"` / `"patent"` / `"web"` |
| `**kwargs` | — | — | 传递给对应搜索函数 |
...
#### `search_research_outputs(query, sources: str | list[str] | None = None, per_page=10, **kwargs)` → `list[SearchResponse]`

并发搜索数据集、软件、文本、事件等非论文科研产出。默认源来自 registry
`research_output:search`，当前为 `datacite`；也可显式指定 `figshare`。返回 `ResearchOutputResult`，保留 source 的
`resource_type_general`、`resource_type`、rights、相关标识、funding 和声明资源链接，不能视作论文或
下载授权。

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `query` | `str` | — | 搜索关键词；会先 `strip()`，strip 后不能为空 |
| `sources` | `str \| list[str] \| None` | `null`（registry `research_output:search`） | 数据源或数据源列表；单个字符串会归一化为单元素列表 |
| `per_page` | `int` | `10` | 每个源返回结果数 |
...
当前 importer 只支持 `gutenberg`。canonical bulk input 是 Project Gutenberg 官方 daily
...
`open_library`；Gutenberg、DOAB、OAPEN、Internet Archive、Library of Congress、LibriVox 和 Wikisource 都必须显式传入
...
只读取同一记录的 `oai_dc` / `mets` metadata，不请求其 bitstream。OAPEN 同样通过独立的官方
OAI-PMH Books set 运行有界 metadata harvest，保留 funding、逐本 license 和声明 file links；不与
DOAB record 或 rights 混用。LibriVox search 只返回书目与声明的
...
要读取一个明确页面及有界 revision，请使用 Python API `get_wikisource_page_detail()`，而不是假定
存在 REST detail endpoint。

`sources=gutenberg` 只查询已完成导入的本地 SQLite catalog；它不会把搜索请求转发给
Project Gutenberg。导入采用官方 RDF/XML metadata（bulk canonical input 为
...
下载；Wikisource 的 `content_format`、正文 size / truncation、页面与 revision provenance，以及
站点贡献许可和底本 rights 都是不同字段/证据层。不会从外部链接或站点托管本身推断下载、全球
...
搜索数据集、软件、文本、活动、报告等非论文科研产出。默认 source 由 registry
`research_output:search` 派生，当前为匿名 DataCite REST API；可显式指定匿名 Figshare public API v2。每条结果保留
`resource_type_general`（受控类型）和 `resource_type`（仓储的具体类型），不会强制转换为
`PaperResult`。响应中的 `creators`、`contributors`、`dates`、`subjects`、`descriptions`、
`funding_references`、`rights_list`、`related_identifiers`、`geo_locations`、`identifiers`、
`landing_url`、`content_urls` 和 `resources` 都是 metadata/provenance 字段。

DataCite 的 `landing_url` / `content_urls` 与 Figshare 的 `resources`（包括声明 download URL 和
`is_link_only`）都不证明目标可访问、可抓取、可下载或可再分发；SouWen 不跟随这些链接，不下载内容。
`rights_list` 会保留多个上游权利/许可陈述，不能被降格为单个“开放”布尔值。

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `q` | string (1-500) | *(必填)* | 搜索关键词；会先 `strip()`，strip 后不能为空 |
| `sources` | string \| null | `null`（registry `research_output:search`） | 数据源列表，逗号分隔 |
| `per_page` | int (1-100) | `10` | 每个数据源返回结果数 |
| `timeout` | float (1-300) \| null | `null` | 端点硬超时（秒），超时返回 504 |
