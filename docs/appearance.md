...
当前内置 5 个皮肤：`souwen-google`（默认）、`souwen-nebula`、`carbon`、`apple`、`ios`。

### souwen-google（默认）

Google 科技炫彩风格，参照 Material Design 3 设计语言，追求现代感与活力。

- **设计理念**：Google Material Design 3 的层级与色彩体系 + Apple HIG 的克制质感
- **视觉特征**：大圆角卡片、柔和弥散阴影、毛玻璃效果、Framer Motion 动效、Google 四色品牌标识
- **布局**：左侧渐变导航栏 + 顶部标题栏 + 主内容区
- **配色方案**：Google（靛蓝紫）、极光 Aurora（青碧绿）、黑曜石 Obsidian（石板灰）
...
- **设计理念**：初代 SouWen 默认皮肤，适合喜欢低调学术风格的用户
- **视觉特征**：标准卡片布局、简洁边框、经典导航栏
- **布局**：左侧导航栏 + 顶部标题栏 + 主内容区
...
- **布局**：顶部全宽导航栏（与 souwen-google 的侧边栏完全不同）
...
| 属性 | 浅色模式 | 深色模式 |
|------|----------|----------|
| 主强调色 | `#1a73e8` (Google Blue) | `#8ab4f8` (浅蓝) |
...
当使用 `VITE_SKINS=all` 或 `VITE_SKINS=souwen-google,souwen-nebula,carbon,apple,ios` 等多皮肤值构建时，面板会在标题栏显示「切换皮肤」按钮。
Docker 构建默认包含所有皮肤（`souwen-google` / `souwen-nebula` / `carbon` / `apple` / `ios`），开箱即支持运行时切换。

切换皮肤后页面会自动刷新，选择保存在 localStorage 中。

#### 构建时选择（单皮肤构建）

使用 `VITE_SKINS=souwen-google` 构建时，仅包含指定皮肤，体积更小，不显示切换按钮。
...
# 单皮肤开发（仅 google / nebula / carbon 提供了快捷脚本）
npm run dev:google             # 仅 souwen-google
npm run dev:nebula             # 仅 souwen-nebula
...
# 任意组合
VITE_SKINS=souwen-google,apple npm run dev
...
# 单皮肤构建（体积更小，google / nebula / carbon 提供快捷脚本）
...
# 本地开发使用 build:local 会额外复制到 src/souwen/server/panel.html
# Docker 构建由 Dockerfile 中的 COPY 指令完成此步骤
```
...
# 仅 classic 皮肤（体积更小）
docker build --build-arg SKINS=souwen-google -t souwen .
...
| Docker build | `SKINS` | `--build-arg` | `docker build --build-arg SKINS=souwen-google .` |
...
```bash
cp -r panel/src/skins/souwen-google panel/src/skins/my-skin
```
...
```typescript
const ALL_SKINS = ['souwen-google', 'souwen-nebula', 'carbon', 'apple', 'ios', 'my-skin']
```
...
# 多皮肤开发（与其他皮肤一起）
VITE_SKINS=souwen-google,my-skin npm run dev
...
html[data-skin='souwen-google']                         { --bg: #f8f9fb; }
html[data-skin='souwen-google'][data-mode='dark']       { --bg: #0a0a0f; }
html[data-skin='souwen-google'][data-scheme='aurora']   { --accent: #0d9488; }
...
| 键名 | 值 | 说明 |
|------|-----|------|
| `souwen_skin` | `souwen-google` / `souwen-nebula` / `carbon` / `apple` / `ios` | 当前皮肤（多皮肤模式） |
| `souwen_mode` | `light` / `dark` | 明暗模式 |
| `souwen_scheme` | souwen-google：`google` / `aurora` / `obsidian`<br>souwen-nebula：`nebula` / `aurora` / `obsidian`<br>carbon：`terminal` / `matrix` / `ember`<br>apple：`blue`<br>ios：`default` | 配色方案（按当前皮肤可选值取值） |
...
```tsx
// skins/souwen-google/pages/SearchPage.tsx
