...
[Souwen v2rc3 发布候选门禁](./internal/rc-readiness-gates.md)。该门禁文档只固定规则；
...
- `release` 运行 source、clean install、Panel、container，以及精确四个平台的
  ...
  `souwen-openapi-2.0.0rc3.json`。Assembler 只接受四个固定名称的 archive、对应的四份
...
前缀，旧 binary 不得出现在 RC3 manifest 或 Release assets。Tag 与 prerelease 只能由 central
workflow 的 publish job 创建。`publish=true` 只在 owner-approved RC3 exact-main release 中使用。

## RC3 PyInstaller Server bundle

`.github/workflows/build-pyinstaller-server.yml` 是 RC3 四平台 Server bundle builder。它既可由
...
```text
souwen-server-2.0.0rc3-linux-amd64.tar.gz
souwen-server-2.0.0rc3-linux-arm64.tar.gz
souwen-server-2.0.0rc3-macos-arm64.tar.gz
souwen-server-2.0.0rc3-windows-amd64.zip
```
...
libraries。RC3 不把一个不含 browser runtime的裸 executable描述为 self-contained bundle。
...
- RC3 HFS 使用 `deploy/process/supervisor.py` 管理两个进程：Browser Worker 只绑定
