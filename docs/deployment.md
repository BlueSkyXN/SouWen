...
[Souwen v2rc4 发布候选门禁](./internal/rc-readiness-gates.md)。该门禁文档只固定规则；
...
- `release` 运行 source、clean install、Panel、container，以及精确四个平台的
  ...
  `souwen-openapi-2.0.0rc4.json`。Assembler 只接受四个固定名称的 archive、对应的四份
...
前缀，旧 binary 不得出现在 RC4 manifest 或 Release assets。Tag 与 prerelease 只能由 central
workflow 的 publish job 创建。`publish=true` 只在 owner-approved RC4 exact-main release 中使用。

## RC4 PyInstaller Server bundle

`.github/workflows/build-pyinstaller-server.yml` 是 RC4 四平台 Server bundle builder。它既可由
...
libraries。RC4 不把一个不含 browser runtime的裸 executable描述为 self-contained bundle。
...
- RC4 HFS 使用 `deploy/process/supervisor.py` 管理两个进程：Browser Worker 只绑定
