...
[Souwen v2rc6 发布候选门禁](./internal/rc-readiness-gates.md)。未经明确批准，不得同步 Space
...
| `SOUWEN_CONFIG_B64` | 与 Space 同源的 RC6 配置；promotion 前验证 exact 一个 LLM Search Provider、gateway 结构与一次 single-attempt live evidence | 仅 workflow 内存/临时文件预检；不输出配置值，post-deploy 仍重新验证 HFS runtime |
| `UNIAPI_API_KEY` | RC6 UniAPI gateway credential | workflow 将其写入同名 Space Secret；只回读 Secret 名称，不读取或记录值 |
