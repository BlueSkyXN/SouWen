...
而应由发布流水线写入候选专属的 `release-manifest.json` artifact。
...
- **RC-ready**：除 HFS promotion 外的 15 个 gate 全部 `PASS`；HFS 为
  `required=false, status=NOT_RUN`。Central workflow 使用 `publish=false, deploy_hfs=false`，
  只生成 evidence bundle，不创建 tag/Release、不写 Space。
...
- **RC-ready Go**：15 个 always-required gate 全部 `PASS`，HFS 明确为未请求的 conditional
  `NOT_RUN`，manifest/candidate/verifier SHA、payload/envelope inventory 与 checksum 一致。
