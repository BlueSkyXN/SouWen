...
CR-03 的同步接口继续调用 `socket.getaddrinfo(host, None, AF_UNSPEC, SOCK_STREAM)`，没有 timeout、retry
或 cooperative cancellation 参数。CR-09 另增 `resolve_fetch_target_async(url)`，只通过
`asyncio.to_thread(resolve_fetch_target, url)` 让 async caller 不阻塞 event loop；同步 resolver 仍是所有
安全规则和 reason 的唯一真源。等待 task 被取消或外层 `wait_for()` 超时时，caller 可立即恢复，但已经
开始的 executor worker 和系统 DNS 调用不会被终止，仍运行至自行返回。该接口不宣称 hard DNS
deadline，不新增 timeout 参数、cache、retry、executor policy、resolver policy 或多 IP retry。
...
CR-09 只让 `BaseScraper` 与 builtin fetch redirect loop await canonical async wrapper；两者仍消费同一
`ResolvedFetchTarget` 并继续使用 IP literal `connect_url`、原始 `Host` 与 HTTPS SNI。其他同步 validator
call sites、legacy `validate_fetch_url` helper patch 以及 local fixture harness 对 canonical sync resolver 的
patch 保持。Builtin 不承诺继续响应对 `souwen.web.fetch.resolve_fetch_target` 的独立 monkeypatch；不能为
保留测试私有 patch 而让 Common Runtime 反向依赖 legacy Web。取消发生在 DNS wait 时不得启动 HTTP
transport；worker 后续返回也不得让已取消 task 发出 stale request。Rollback 只恢复两名 consumer 的
同步调用并移除 async export，不涉及数据或配置迁移。
...
| VAL-CR-040 | sync/async resolver 对 target/reason、mixed DNS、fake-IP、IDNA、Host/SNI 完全 parity |
| VAL-CR-041 | async DNS 在 executor thread 运行，event loop 保持 responsive |
| VAL-CR-042 | task cancellation/outer timeout 停止等待，但测试明确证明 running DNS thread 继续至释放 |
...
1. 可终止 resolver worker 的 hard DNS deadline；CR-09 只提供 cancellable async wait，不伪报底层终止；
