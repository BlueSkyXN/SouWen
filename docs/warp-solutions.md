...
如果 Docker 镜像或系统中没有预装所需二进制（如 usque、wireproxy、wgcf），可以通过以下方式在运行时安装：

- **Web 面板**：WarpPage → 组件安装器卡片 → 点击"安装"按钮
- **API**：`POST /admin/warp/components/install?component=usque`
- **安装目录**：`/app/data/bin/`（自动加入 PATH，不影响系统预装）
- **代理支持**：设置 `GH_PROXY` 环境变量可通过镜像站下载（如 `https://ghgo.xyz/`）
