...
| Profile | 覆盖内容 | 运行位置 |
|---|---|---|
| `minimal` | 源码 CLI 的 help/version/sources/config 等无网络入口 | `V2 CI / v2 server + minimal profile`、`HF Space CD / API surface and source CLI` |
| `server` | `tests/test_server` 与 `tests/test_hf_space_smoke.py` 的本地 API/smoke 契约 | `V2 CI / v2 server + minimal profile`、`HF Space CD / API surface and source CLI` |
| `full` | 全部 source、doctor、plugin、fetch handler 的 import surface | `V2 CI / v2 full runtime profile`、`CI / 测试 (Python 3.11, ubuntu-latest)` |
...
`v2-dev` 使用独立 `V2 CI` 作为 v2 长期集成和发布前 gate，不直接复用 `main`
上的生产 CD 触发面。当前 `V2 CI` 必须覆盖：

- bootstrap gate：registry/docs 测试、`tools/gen_docs.py --check`、import surface
  单测、wheel surface 检查和 registry baseline 输出。
- full pytest matrix：Ubuntu Python 3.10/3.11/3.12/3.13，以及 macOS/Windows
  Python 3.11。
- server + minimal profile：安装 `.[dev,server]` 后运行 `server` 和 `minimal`
  profile，并上传 JSON/Markdown report。
- full runtime profile：安装发布前完整运行时依赖后运行 `full` profile，覆盖全部
  source、doctor、plugin 与 fetch handler import surface，并上传 JSON/Markdown report。
...
这些 gate 是 v2 candidate 合回 `main` 前的必过项。它们不负责生产部署、PyPI
发布或二进制 release 产物上传。

`External Smoke Gate` 是外部能力的 nightly / release gate 入口：

- `workflow_dispatch`：手动跑当前 PR required 外部 gate。
- `schedule`：每天 02:17 Asia/Shanghai 跑 nightly；失败时创建或更新带 `ci:external` / `smoke-failure` label 的 issue，恢复后自动关闭。
- tag `v*`：作为 release gate 跑 Scrapling / Crawl4AI 真实运行时 gate。
- 每个 gate job 上传 JSON + Markdown artifact，JSON 仍是 source of truth。

v2 发布前，`External Smoke Gate` 不要求每个 `v2-dev` PR 自动运行；它应在候选
版本或 mergeback PR 上手动以 `suite=release` 跑一次，并把 required `FAIL`
视为发布阻断。
