...
def render() -> str:
    ...
    lines.append(
        ...
        "`SourceAdapter` 同时驱动 REST API、doctor、Provider runtime 和 Panel 视图。"
    )
    lines.append("")
    lines.append("## 如何阅读")
    lines.append("")
    ...
    lines.append(
        "- `/api/v1/sources`、doctor 和 Panel 使用同一份公开 Source Catalog："
        "`sources[]` 保留全部公开条目，并用 `category`、`domain`、`capabilities`、"
        "`available` 汇总启用状态、配置有效性、凭据与本地 runtime readiness；"
        "`runtime_available` / `runtime_reason` 单独解释依赖 importability。"
    )
    lines.append(
    ...
    lines.append(
        ...
        "`/api/v1/sources[].available` 只表示当前启用、配置、凭据和本地 runtime 条件满足，"
        "不证明上游此刻可达。Catalog 和 doctor 的 `runtime_available` / "
        "`runtime_reason` 会单独解释依赖/importability；"
