...
from souwen.providers.fetch_sources.scraperapi import adapter
...
@pytest.mark.asyncio
async def test_scraperapi_fetch_maps_safe_receipt_and_fails_closed(monkeypatch):
    monkeypatch.setattr(
        adapter, "validate_fetch_url", lambda url: (not url.endswith("/private"), "")
    )
    client = _Client(_receipt())
    result = await ScraperAPIFetchProvider(client).fetch(
        FetchTargetRequest(target="https://1.1.1.1/page"),
        RequestContext(request_id="scraperapi"),
    ...
    with pytest.raises(ProviderError) as caught:
        await ScraperAPIFetchProvider(blocked).fetch(
    ...
    with pytest.raises(ProviderError) as caught:
        await ScraperAPIFetchProvider(_Client(_receipt("https://1.1.1.1/private"))).fetch(
            FetchTargetRequest(target="https://1.1.1.1/page"),
...
def test_scraperapi_static_declarations_agree():
    assert (
        validate_spec_manifest(SCRAPERAPI_FETCH_PROFILE, SCRAPERAPI_PROVIDER_MANIFEST)
        is SCRAPERAPI_FETCH_PROFILE
    )
    assert SCRAPERAPI_PROVIDER_MANIFEST.secrets.references == ("SCRAPERAPI_API_KEY",)
