...
def test_rc3_server_bundle_workflow_is_a_read_only_reusable_proof_builder() -> None:
...
def test_rc3_server_bundle_workflow_has_exact_native_four_archive_matrix() -> None:
    text = _workflow_text(".github/workflows/build-pyinstaller-server.yml")
    expected = {
        "ubuntu-24.04": "souwen-server-2.0.0rc3-linux-amd64.tar.gz",
        "ubuntu-24.04-arm": "souwen-server-2.0.0rc3-linux-arm64.tar.gz",
        "macos-15": "souwen-server-2.0.0rc3-macos-arm64.tar.gz",
        "windows-2025": "souwen-server-2.0.0rc3-windows-amd64.zip",
    }

    assert text.count("archive: souwen-server-2.0.0rc3-") == 4
...
def test_rc3_server_bundle_builds_tracked_target_onedir_with_bundled_chromium() -> None:
    text = _workflow_text(".github/workflows/build-pyinstaller-server.yml")
...
def test_server_bundle_openapi_checksum_uses_the_verified_canonical_artifact() -> None:
    ...
    assert "contracts/openapi/souwen-openapi-2.0.0rc3.json" in checksum_step
