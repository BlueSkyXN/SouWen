...
from scripts import oapen_functional_check as check
...
def test_verify_oapen_registered_reports_explicit_experimental_contract() -> None:
    message, details = check.verify_oapen_registered()

    assert message == "oapen registry contract passed"
    assert details == {
        "source": "oapen",
        "domain": "book",
...
async def test_search_and_detail_only_use_same_record_metadata_flow() -> None:
    class FakeClient:
        ...
        async def search(self, query: str, *, per_page: int) -> SearchResponse:
            ...
            return SearchResponse(
                ...
                results=[
                    BookResult(
                        source="oapen",
                        source_record_id="20.500.12657/1234",
                        title="Search result",
                        source_url="https://library.oapen.org/handle/20.500.12657/1234",
                    )
                ],
            )
        ...
        async def get_by_id(self, record_id: str, *, file_limit: int) -> BookResult:
            ...
            access = ResourceAccess(
                status="open_access", license_url="https://creativecommons.org/licenses/by/4.0/"
            )
            resource = ResourceLink(
                url="https://library.oapen.org/bitstream/20.500.12657/1234/1/book.pdf",
                relation="bitstream",
                source="oapen",
                access=access,
            )
    ...
    message, details = await check.run_oapen_search_and_detail(
        query="Wissenschaft", per_page=1, file_limit=3, client_factory=FakeClient
    )

    assert message == "OAPEN OAI search/detail returned record 20.500.12657/1234"
    assert details["funding_count"] == 1
    assert details["bitstream_count"] == 1
    assert details["no_automatic_file_download"] is True
    assert details["oapen_and_doab_records_are_not_interchanged"] is True
    assert FakeClient.calls == [
        ("search", ("Wissenschaft", 1)),
        ("get_by_id", ("20.500.12657/1234", 3)),
    ]


def test_offline_mode_writes_skip_report_without_network(tmp_path: Path) -> None:
    json_report = tmp_path / "oapen.json"
    markdown_report = tmp_path / "oapen.md"

    completed = subprocess.run(
        [
            sys.executable,
            "scripts/oapen_functional_check.py",
            "--mode",
            "offline",
            "--json-report",
            str(json_report),
    ...
    assert report["script"] == "oapen_functional_check"
...
async def test_live_failures_are_warn_only_unless_required(
    ...
    monkeypatch.setattr(check, "verify_oapen_registered", lambda: ("ok", {}))
    monkeypatch.setattr(check, "run_oapen_search_and_detail", failing_live_check)
