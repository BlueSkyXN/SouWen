...
from souwen.providers.fetch_sources.apify import adapter
...
@pytest.mark.asyncio
async def test_apify_fetch_maps_safe_receipt_and_fails_closed(monkeypatch):
    monkeypatch.setattr(
        adapter, "validate_fetch_url", lambda url: (not url.endswith("/private"), "")
    )
    client = _Client(_receipt())
    result = await ApifyFetchProvider(client).fetch(
        FetchTargetRequest(target="https://1.1.1.1/page"),
        RequestContext(request_id="apify"),
    ...
    with pytest.raises(ProviderError) as caught:
        await ApifyFetchProvider(blocked).fetch(
    ...
    with pytest.raises(ProviderError) as caught:
        await ApifyFetchProvider(_Client(_receipt("https://1.1.1.1/private"))).fetch(
            FetchTargetRequest(target="https://1.1.1.1/page"),
...
def test_apify_static_declarations_agree():
    assert (
        validate_spec_manifest(APIFY_FETCH_PROFILE, APIFY_PROVIDER_MANIFEST) is APIFY_FETCH_PROFILE
    )
    assert APIFY_PROVIDER_MANIFEST.secrets.references == ("APIFY_API_TOKEN",)
