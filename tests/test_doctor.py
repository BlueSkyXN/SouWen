...
验证：85 个数据源的完整性检查、状态判断（ok/missing_key/limited/unavailable/warning）、
报告格式化与符号呈现、以及 Tier 分层显示。

测试清单：
- ``TestCheckAll``：check_all() 返回 85 源、必要字段完整性、Key 配置状态检测
...
class TestCheckAll:
    ...
    def test_source_config_matches_37(self):
        """source registry 有 85 个数据源"""
        assert len(get_all_sources()) == 85
