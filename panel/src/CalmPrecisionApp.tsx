...
function SearchPage() {
  ...
  return <><Header eyebrow="Search" title="搜索" description="选择领域并提交查询，结果统一规范化，只展示可追溯的公共字段。" /><section className={styles.panel}><form className={styles.form} onSubmit={submit}><div className={styles.field}><label className={styles.label} htmlFor="search-query">查询</label><input className={styles.input} id="search-query" value={query} onChange={(event) => setQuery(event.target.value)} required maxLength={4096} placeholder="输入研究问题、主题或作者" /></div><fieldset className={styles.field}><legend className={styles.label}>领域</legend><div className={styles.choiceList}>{DOMAINS.map((domain) => <label className={styles.choice} key={domain}><input type="checkbox" checked={domains.includes(domain)} onChange={() => toggle(domain)} />{domain}</label>)}</div><span className={styles.hint}>至少选择一个领域。</span></fieldset><div className={styles.formFooter}><span className={styles.hint}>由 generated SouWenClient 发送请求。</span><button className={styles.button} disabled={state.status === 'loading' || !query.trim() || !domains.length} type="submit">运行搜索</button></div></form></section><Status state={state} name="搜索结果" />{state.status === 'success' && <Results items={state.data?.items ?? []} />}</>
}
...
function FetchPage() {
  ...
  return <><Header eyebrow="Fetch" title="网页抓取" description="每行一个 URL，最多 20 个。抓取与 robots 策略全部由服务端执行。" /><section className={styles.panel}><form className={styles.form} onSubmit={submit}><div className={styles.field}><label className={styles.label} htmlFor="fetch-targets">URLs</label><textarea className={styles.textarea} id="fetch-targets" value={targets} onChange={(event) => setTargets(event.target.value)} required placeholder={'https://example.org/article\nhttps://example.org/report'} /><span className={styles.hint}>仅接受 http/https URL，服务端负责最终校验。</span></div><div className={styles.field}><label className={styles.label} htmlFor="fetch-strategy">Provider 策略</label><select className={styles.select} id="fetch-strategy" value={strategy} onChange={(event) => setStrategy(event.target.value as typeof strategy)}><option value="fallback">fallback</option><option value="fanout">fanout</option></select></div><div className={styles.formFooter}><span className={styles.hint}>可用性取决于当前角色和 provider 配置。</span><button className={styles.button} disabled={state.status === 'loading' || !targets.trim()} type="submit">开始 Fetch</button></div></form></section><Status state={state} name="Fetch 结果" />{state.status === 'success' && <Results items={state.data?.items ?? []} />}</>
}
...
function ProvidersPage() {
  ...
  return <><Header eyebrow="Providers" title="Provider 目录" description="展示各 provider 的能力、可用状态和缺失字段，不回显连接配置或密钥。"><button className={styles.secondaryButton} onClick={() => void load()} disabled={state.status === 'loading'}>刷新</button></Header><Status state={state} name="Provider 目录" />{state.status === 'success' && (!state.data?.items.length ? <div className={`${styles.panel} ${styles.empty}`}>没有可用 Provider。</div> : <section className={styles.resultList} aria-live="polite">{state.data.items.map((item, index) => <article className={`${styles.panel} ${styles.provider}`} key={String(item.provider ?? index)}><div><h2 className={styles.providerName}>{String(item.provider ?? 'unknown')}</h2><p className={styles.providerMeta}>{Array.isArray(item.capabilities) ? item.capabilities.join(' · ') : '未声明能力'}{Array.isArray(item.missing_fields) && item.missing_fields.length ? ` · 缺失：${item.missing_fields.join(', ')}` : ''}</p></div><span className={`${styles.badge} ${item.availability === 'available' ? styles.badgeAvailable : ''}`}>{String(item.reason ?? item.availability ?? 'unknown')}</span></article>)}</section>)}</>
}
...
function LoginPage() {
  ...
  return <div className={styles.login}><section className={`${styles.panel} ${styles.loginCard}`} aria-labelledby="login-title"><header className={styles.loginHeader}><p className={styles.eyebrow}>SouWen</p><h1 className={styles.loginTitle} id="login-title">连接服务器</h1><p className={styles.loginNote}>令牌只发送给同源、loopback 或明确允许的 API 地址，不会用于提权。无密码管理员访问仅在服务端开启 SOUWEN_ADMIN_OPEN=1 时可用。</p></header><Status state={state} name="连接" /><form className={styles.form} onSubmit={connect}><div className={styles.field}><label className={styles.label} htmlFor="server-url">Server URL</label><input className={styles.input} id="server-url" required value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} /></div><div className={styles.field}><label className={styles.label} htmlFor="access-token">访问令牌</label><input className={styles.input} id="access-token" type="password" value={token} onChange={(event) => setToken(event.target.value)} autoComplete="current-password" /><span className={styles.hint}>服务端未开放无密码访问时，输入对应角色的令牌。</span></div><label className={styles.choice}><input type="checkbox" checked={remember} onChange={(event) => setRemember(event.target.checked)} />在此设备保留会话</label><button className={styles.button} type="submit" disabled={state.status === 'loading'}>连接</button></form></section></div>
}
