...
function SearchPage() {
  ...
  return <><Header eyebrow="Search" title="检索，保持聚焦" description="在一个规范化请求中组合所需领域，结果只展示可追溯的公共字段。" /><section className={styles.panel}><form className={styles.form} onSubmit={submit}><div className={styles.field}><label className={styles.label} htmlFor="search-query">查询</label><input className={styles.input} id="search-query" value={query} onChange={(event) => setQuery(event.target.value)} required maxLength={4096} placeholder="输入研究问题、主题或作者" /></div><fieldset className={styles.field}><legend className={styles.label}>领域</legend><div className={styles.choiceList}>{DOMAINS.map((domain) => <label className={styles.choice} key={domain}><input type="checkbox" checked={domains.includes(domain)} onChange={() => toggle(domain)} />{domain}</label>)}</div><span className={styles.hint}>至少选择一个领域。</span></fieldset><div className={styles.formFooter}><span className={styles.hint}>请求由 generated SouWenClient 发送。</span><button className={styles.button} disabled={state.status === 'loading' || !query.trim() || !domains.length} type="submit">运行搜索</button></div></form></section><Status state={state} name="搜索结果" />{state.status === 'success' && <Results items={state.data?.items ?? []} />}</>
}
...
function LlmSearchPage() {
  ...
  return <><Header eyebrow="LLM Search" title="带证据的综合检索" description="明确选择 LLM Search provider 与策略；回答、证据和使用量由服务端规范化。" /><section className={styles.panel}><form className={styles.form} onSubmit={submit}><div className={styles.field}><label className={styles.label} htmlFor="llm-query">问题</label><textarea className={styles.textarea} id="llm-query" value={query} onChange={(event) => setQuery(event.target.value)} required maxLength={4096} placeholder="提出一个需要依据的研究问题" /></div><div className={styles.formRow}><div className={styles.field}><label className={styles.label} htmlFor="llm-providers">Provider IDs</label><input className={styles.input} id="llm-providers" value={providers} onChange={(event) => setProviders(event.target.value)} required placeholder="例如: research-llm" /><span className={styles.hint}>多个 provider 以逗号分隔；可在 Providers 页面核对可用项。</span></div><div className={styles.field}><label className={styles.label} htmlFor="llm-strategy">策略</label><select className={styles.select} id="llm-strategy" value={strategy} onChange={(event) => setStrategy(event.target.value as typeof strategy)}><option value="first_success">first_success</option><option value="fanout">fanout</option><option value="single">single</option></select></div></div><div className={styles.formFooter}><span className={styles.hint}>服务端预算与认证边界保持生效。</span><button className={styles.button} disabled={state.status === 'loading' || !query.trim() || !providers.trim()} type="submit">综合回答</button></div></form></section><Status state={state} name="LLM Search" />{state.status === 'success' && <section className={styles.resultList} aria-live="polite">{state.data?.answer && <article className={`${styles.panel} ${styles.result}`}><h2 className={styles.sectionTitle}>回答</h2><p className={styles.resultBody}>{state.data.answer}</p></article>}<Results items={state.data?.evidence ?? state.data?.items ?? []} /></section>}</>
}
...
function FetchPage() {
  ...
  return <><Header eyebrow="Fetch" title="受策略约束的内容提取" description="每行一个 URL，最多 20 个。浏览器不处理 SSRF 或 robots 策略，服务端仍是唯一执行与安全边界。" /><section className={styles.panel}><form className={styles.form} onSubmit={submit}><div className={styles.field}><label className={styles.label} htmlFor="fetch-targets">URLs</label><textarea className={styles.textarea} id="fetch-targets" value={targets} onChange={(event) => setTargets(event.target.value)} required placeholder={'https://example.org/article\nhttps://example.org/report'} /><span className={styles.hint}>仅接受 http/https URL；服务端会执行最终验证。</span></div><div className={styles.field}><label className={styles.label} htmlFor="fetch-strategy">Provider 策略</label><select className={styles.select} id="fetch-strategy" value={strategy} onChange={(event) => setStrategy(event.target.value as typeof strategy)}><option value="fallback">fallback</option><option value="fanout">fanout</option></select></div><div className={styles.formFooter}><span className={styles.hint}>Fetch 可能受当前角色和 provider 配置限制。</span><button className={styles.button} disabled={state.status === 'loading' || !targets.trim()} type="submit">开始 Fetch</button></div></form></section><Status state={state} name="Fetch 结果" />{state.status === 'success' && <Results items={state.data?.items ?? []} />}</>
}
...
function ProvidersPage() {
  ...
  return <><Header eyebrow="Providers" title="可用性，不是密钥配置" description="目录只展示规范化的 provider 能力、可用状态和缺失字段名称；不会回显连接配置或 secret。"><button className={styles.secondaryButton} onClick={() => void load()} disabled={state.status === 'loading'}>刷新</button></Header><Status state={state} name="Provider 目录" />{state.status === 'success' && (!state.data?.items.length ? <div className={`${styles.panel} ${styles.empty}`}>没有可用 Provider。</div> : <section className={styles.resultList} aria-live="polite">{state.data.items.map((item, index) => <article className={`${styles.panel} ${styles.provider}`} key={String(item.provider ?? index)}><div><h2 className={styles.providerName}>{String(item.provider ?? 'unknown')}</h2><p className={styles.providerMeta}>{Array.isArray(item.capabilities) ? item.capabilities.join(' · ') : '未声明能力'}{Array.isArray(item.missing_fields) && item.missing_fields.length ? ` · 缺失：${item.missing_fields.join(', ')}` : ''}</p></div><span className={`${styles.badge} ${item.availability === 'available' ? styles.badgeAvailable : ''}`}>{String(item.reason ?? item.availability ?? 'unknown')}</span></article>)}</section>)}</>
}
...
function AdminPage({ kind }: { kind: 'runtime' | 'settings' }) {
  ...
  return <><Header eyebrow={kind === 'runtime' ? 'Runtime' : 'Settings'} title={title} description={kind === 'runtime' ? '仅管理员可查看的运行诊断摘要；此页面不触发联网探测或状态变更。' : '仅展示访问策略、Provider 配置数量和 LLM Search 姿态；不回显完整配置，也不提供 secret 编辑。'}><div className={styles.topbarActions}><button className={styles.secondaryButton} onClick={() => navigate('/runtime')} aria-pressed={kind === 'runtime'}>Runtime</button><button className={styles.secondaryButton} onClick={() => navigate('/settings')} aria-pressed={kind === 'settings'}>Settings</button><button className={styles.secondaryButton} onClick={() => void load()} disabled={state.status === 'loading'}>刷新</button></div></Header><Status state={state} name={title} />{state.status === 'success' && <section className={styles.panel}><pre className={styles.mono} aria-label={`${title} JSON`}>{JSON.stringify(state.data, null, 2)}</pre></section>}</>
}

function Shell() {
  const logout = useAuthStore((state) => state.logout); const role = useAuthStore((state) => state.role); const baseUrl = useAuthStore((state) => state.baseUrl)
  ...
  return <div className={styles.app}><div className={styles.shell}><aside className={styles.sidebar}><div className={styles.brand}><span className={styles.mark}>SW</span><span>SouWen</span></div><nav className={styles.navigation} aria-label="主导航">{nav.map(({ to, label, Icon }) => <NavLink className={({ isActive }) => `${styles.navLink} ${isActive ? styles.navLinkActive : ''}`} key={to} to={to}><Icon size={17} aria-hidden="true" />{label}</NavLink>)}</nav><div className={styles.sideFooter}><span className={styles.identity} title={baseUrl || 'same-origin'}>{role} · {baseUrl || 'same-origin'}</span><button className={styles.secondaryButton} onClick={logout}><LogOut size={15} aria-hidden="true" /> 登出</button></div></aside><main className={styles.main}><header className={styles.topbar}><p className={styles.topbarTitle}>Calm Precision Panel</p><div className={styles.topbarActions}><button className={styles.iconButton} onClick={toggleTheme} aria-label={dark ? '切换为浅色模式' : '切换为深色模式'}>{dark ? <Sun size={17} /> : <Moon size={17} />}</button></div></header><div className={styles.content}><Outlet /></div></main></div></div>
}
...
function LoginPage() {
  ...
  return <div className={styles.login}><section className={`${styles.panel} ${styles.loginCard}`} aria-labelledby="login-title"><header className={styles.loginHeader}><p className={styles.eyebrow}>SouWen</p><h1 className={styles.loginTitle} id="login-title">连接管理面板</h1><p className={styles.loginNote}>令牌只会发送到受校验的同源、loopback 或明确允许的 API 地址。没有令牌不会自动提升权限；服务端仅在 SOUWEN_ADMIN_OPEN=1 时授予无密码管理员访问。</p></header><Status state={state} name="连接" /><form className={styles.form} onSubmit={connect}><div className={styles.field}><label className={styles.label} htmlFor="server-url">Server URL</label><input className={styles.input} id="server-url" required value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} /></div><div className={styles.field}><label className={styles.label} htmlFor="access-token">访问令牌</label><input className={styles.input} id="access-token" type="password" value={token} onChange={(event) => setToken(event.target.value)} autoComplete="current-password" /><span className={styles.hint}>若服务端未开放无密码管理员访问，请输入对应角色的令牌。</span></div><label className={styles.choice}><input type="checkbox" checked={remember} onChange={(event) => setRemember(event.target.checked)} />在此设备保留会话</label><button className={styles.button} type="submit" disabled={state.status === 'loading'}>连接</button></form></section></div>
}
