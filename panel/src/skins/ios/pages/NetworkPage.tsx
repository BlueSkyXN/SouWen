      ...
      <div className={styles.formGroup}>
        ...
        <div className={styles.groupCard}>
          <div className={styles.formRow}>
            <span className={styles.rowLabel}>{t('network.status')}</span>
    ...
    <div className={styles.formGroup}>
      ...
      <div className={styles.groupCard}>
        ...
        {warp.status !== 'disabled' && (
          <>
            <div className={`${styles.formRow} ${styles.formRowSep}`}>
              <span className={styles.rowLabel}>{t('network.mode')}</span>
              <span className={styles.rowValueMuted}>{formatWarpMode(warp.mode, (key) => t(key))}</span>
            </div>
            <div className={`${styles.formRow} ${styles.formRowSep}`}>
              <span className={styles.rowLabel}>{t('network.port')}</span>
              <span className={styles.rowValueMuted}>{warp.socks_port}</span>
            </div>
            {warp.protocol && (
              <div className={`${styles.formRow} ${styles.formRowSep}`}>
                <span className={styles.rowLabel}>{t('warp.protocol')}</span>
                <span className={styles.rowValueMuted}>{warp.protocol}</span>
              </div>
            )}
            {warp.proxy_type && (
              <div className={`${styles.formRow} ${styles.formRowSep}`}>
                <span className={styles.rowLabel}>{t('warp.proxyType')}</span>
                <span className={styles.rowValueMuted}>{warp.proxy_type}</span>
              </div>
            )}
            {warp.http_port > 0 && (
              <div className={`${styles.formRow} ${styles.formRowSep}`}>
                <span className={styles.rowLabel}>{t('warp.httpPort')}</span>
                <span className={styles.rowValueMuted}>{warp.http_port}</span>
              </div>
            )}
            {warp.ip && (
              <div className={`${styles.formRow} ${styles.formRowSep}`}>
                <span className={styles.rowLabel}>{t('network.ip')}</span>
                <span className={styles.rowValueMuted}>{warp.ip}</span>
              </div>
            )}
            {warp.pid > 0 && (
              <div className={`${styles.formRow} ${styles.formRowSep}`}>
                <span className={styles.rowLabel}>{t('network.pid')}</span>
                <span className={styles.rowValueMuted}>{warp.pid}</span>
              </div>
            )}
            {warp.interface && (
              <div className={`${styles.formRow} ${styles.formRowSep}`}>
                <span className={styles.rowLabel}>{t('network.interface')}</span>
        ...
        <div className={`${styles.formRow} ${styles.formRowSep}`}>
          <span className={styles.rowLabel}>{t('warp.availableModes')}</span>
          <span className={styles.rowValueMuted}>{formatAvailableWarpModes(warp, (key) => t(key))}</span>
        </div>
        ...
        {!isActive ? (
          <>
            <div className={`${styles.formRow} ${styles.formRowSep}`}>
              <label className={styles.rowLabel} htmlFor="network-warp-mode">{t('network.mode')}</label>
              <select
            ...
            <div className={`${styles.formRow} ${styles.formRowSep}`}>
              <label className={styles.rowLabel} htmlFor="network-warp-port">{t('network.port')}</label>
              <input
            ...
            <div className={`${styles.formRow} ${styles.formRowSep}`}>
              <label className={styles.rowLabel} htmlFor="network-warp-endpoint">{t('network.endpoint')}</label>
              <input
...
function HttpBackendSection() {
  ...
  return (
    <div className={styles.formGroup}>
      <div className={styles.groupTitle}>{t('network.httpBackend')}</div>
      <div className={styles.groupCard}>
        {/* curl_cffi status */}
        <div className={`${styles.formRow} ${styles.formRowSep}`}>
          ...
          <span className={data.curl_cffi_available ? styles.statusOn : styles.statusWarn} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 15 }}>
            <span className={styles.statusDot} />
            {data.curl_cffi_available ? t('network.curlAvailable') : t('network.curlUnavailable')}
          </span>
        </div>

        {/* Global default */}
        <div className={`${styles.formRow} ${styles.formRowSep}`}>
          <span className={styles.rowLabel}>{t('network.globalDefault')}</span>
          <select
  ...
  return (
    <div className={styles.formGroup}>
      ...
      <div className={styles.groupCard}>
        ...
        <div className={`${styles.formRow} ${styles.formRowSep}`}>
          <label className={styles.rowLabel} htmlFor="network-proxy-global">{t('proxy.globalProxy')}</label>
          <input
        ...
        <div className={`${styles.formRow} ${styles.formRowSep}`} style={{ alignItems: 'flex-start' }}>
          <label className={styles.rowLabel} htmlFor="network-proxy-pool">{t('proxy.proxyPool')}</label>
          <textarea
  ...
  return (
    ...
    >
      <m.div variants={staggerItem}>
        <h1 className={styles.pageTitle}>{t('network.title')}</h1>
      </m.div>
