/**
 * 文件用途：Carbon 皮肤的错误边界组件，捕获 React 组件树中的渲染错误
 *
 * 组件/函数清单：
 *   ErrorBoundary（类）
 ...
 *     - 样式特征：使用 Carbon 皮肤的深色终端风格，JetBrains Mono 等等宽字体
 *
 * 模块依赖：
...
export class ErrorBoundary extends Component<Props, State> {
   ...
   * @returns {State} 返回更新后的状态，标记错误已发生
   */
   ...
   * 在 getDerivedStateFromError 之后调用
   * @param {Error} error - 捕获的错误对象
   * @param {React.ErrorInfo} info - React 提供的额外错误信息（包含组件栈跟踪）
   */
   ...
   * 错误界面包含：错误标题、错误信息描述、重新加载按钮
   * 重新加载时会清除错误状态并跳转到登录页重新初始化
   */
  render() {
