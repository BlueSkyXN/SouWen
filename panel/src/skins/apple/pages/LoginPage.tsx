/**
 * 文件用途：Apple 皮肤的登录页面，处理用户身份验证和服务器连接
 *
 * 组件/函数清单：
 *   LoginPage（函数组件）
...
export function LoginPage() {
  ...
  const [shake, setShake] = useState(false) // 登录失败时的抖动反馈
  const [success, setSuccess] = useState(false) // 登录成功状态
  const [autoConnecting, setAutoConnecting] = useState(true) // 初始化时自动连接中
